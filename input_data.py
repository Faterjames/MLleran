...
def get_files(file_dir):
    ...
    for file in os.listdir(file_dir):   # file就是要读取的照片
        ...
        if name[0] == 'cat':            # 所以只用读取 . 前面这个字符串
            ...
            cars.append(file_dir + file)
            label_cars.append(2)
    print('There are %d cats\nThere are %d dogs' % (len(cats), len(dogs)))

    image_list = np.hstack((cats, dogs, cars))  # 在水平方向平铺合成一个行向量
    label_list = np.hstack((label_cats, label_dogs, label_cars))

    temp = np.array([image_list, label_list])  # 生成一个两行数组列表，大小是2 X 50000  12500 cat 12500 catlabel 12500 dog
    temp = temp.transpose()   # 转置向量，大小变成50000 X 2
